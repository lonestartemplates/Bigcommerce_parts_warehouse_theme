...
{
	var i=0;
	$.post('http://demo.webdesksolution.com//wdsbig/filter_files/brand_fetch.php',{id:category,site:'e26881708a71178538fe5fbc482dc445'},function(data) {
	...
	else
	{
	 $('#SideCategoryShopByRate').hide();
		...
		var minimum = Math.floor(data.price.min);
		var maximum = Math.ceil(data.price.max);
...
{
	var category=$('#cat_id').val();
	var form_data = $('#shivamsoft_filter').serialize()+'&id='+category+'&site=e26881708a71178538fe5fbc482dc445';
	var temp=value;
	var idnew=value;
	if(type!='Rating')
	{
	...
	$.post("http://demo.webdesksolution.com//wdsbig/filter_files/brand_filter.php",form_data,function(data){
		obj= JSON && JSON.parse(data) || $.parseJSON(data);
		data = obj;
